...
namespace Zhoubin.Infrastructure.Common.MongoDb
{
    ...
    public sealed class DocumentObjectStorage : ObjectStorageBase, IObjectStorage
    {
        ...
        public bool Delete<T>(string id) where T : IDocumentEntity, new()
        {
            return Excute(() =>
            {
                return Delete(Builders<T>.Filter.Eq(p => p.Id, new ObjectId(id)));
            });
        }
        ...
        /// <returns>true成功,false失败</returns>
        private bool Delete<T>(FilterDefinition<T> queryDocument) where T : IDocumentEntity, new()
        {
            var result = GetCollection<T>().DeleteMany(queryDocument);
        ...
        public bool Delete<T>(IDictionary<string, object> condition) where T : IDocumentEntity, new()
        {
            ...
            return Excute(() => Delete<T>(ConvertDictionaryToFilterDefinition<T>(condition)));
        }
        ...
        public bool Delete<T>(Expression<Func<T, bool>> @where) where T : IDocumentEntity, new()
        {
            ...
            return Delete(Builders<T>.Filter.Where(@where));
        }
        ...
        public bool Update<T>(T entity) where T : IDocumentEntity, new()
        {
            return Excute(() =>
            {
                var fiter = Builders<T>.Filter.Where(p => p.Id == entity.Id);
                var p1 = GetCollection<T>().Find(fiter).FirstOrDefault();
                ...
                p1.Fill(entity);
                var result = GetCollection<T>().ReplaceOne(fiter,p1);
        ...
        {
            var fiter = Builders<T>.Filter.Where(p => p.Id == new ObjectId(id));
            var result = Excute(() => GetCollection<T>().Find(fiter).FirstOrDefault());           
        ...
        public T FindOneByCondition<T>(IDictionary<string, object> condition) where T : IDocumentEntity, new()
        {
            ...
            var result = Excute(() => GetCollection<T>().Find(ConvertDictionaryToFilterDefinition<T>(condition))).FirstOrDefault();
            return result;
        }
        ...
        public T FindOneByCondition<T>(Expression<Func<T, bool>> @where) where T : IDocumentEntity, new()
        {
            ...
            var result = Excute(() => GetCollection<T>().Find(Builders<T>.Filter.Where(@where))).FirstOrDefault();
        ...
        public bool Any<T>(IDictionary<string, object> condition) where T : IDocumentEntity, new()
        {
            ...
            FilterDefinition<T> filter = DictionaryToFilterDefinition<T>(condition);

            return Excute(() => GetCollection<T>().Find(filter).Any());
        }

        private static FilterDefinition<T> DictionaryToFilterDefinition<T>(IDictionary<string, object> condition) where T : IDocumentEntity, new()
        {
            ...
            if (condition.Count > 0)
            {
                ...
                {
                    list.Add(Builders<T>.Filter.Eq(key, condition[key]));
                }
                filter = Builders<T>.Filter.And(list);
            }
        ...
        public List<T> FindByCondition<T>(IDictionary<string, object> condition, IDictionary<string, bool> sortList) where T : IDocumentEntity, new()
        {
            var result = Excute(() =>
            {
                ...
                var query = collection.Find(ConvertDictionaryToFilterDefinition<T>(condition));
        ...
        public bool DeleteAll<T>() where T : IDocumentEntity, new()
        {
            return Excute(() =>
            {
                var result = GetCollection<T>().DeleteMany(Builders<T>.Filter.Empty);
        ...
        {
            return Excute(() => condition == null || condition.Count == 0 ? GetCollection<T>().AsQueryable().Count() : GetCollection<T>().Count(ConvertDictionaryToFilterDefinition<T>(condition)));
        }
        ...
        public long Count<T>(Expression<Func<T, bool>> @where) where T : IDocumentEntity, new()
        {
            ...
            return Excute(() => GetCollection<T>().Count(Builders<T>.Filter.Where(@where)));
        }
        ...
        public List<object> FindByCondition<T>(IDictionary<string, object> condition, IDictionary<string, bool> sortList, Func<IQueryable<T>, List<object>> selectorFunc) where T : IDocumentEntity, new()
        {
            return Excute(() =>
            {
                ...
                return selectorFunc(query.ToList().AsQueryable());
            });
        }
        ...
        //        if (where != null)
        //        {
        //            return query.Where(where).GroupBy(grouprby).Select(selector).ToList();
        //        }
        ...
        public void Advance<T>(Action<IMongoCollection<T>> action) where T : IDocumentEntity, new()
        {
