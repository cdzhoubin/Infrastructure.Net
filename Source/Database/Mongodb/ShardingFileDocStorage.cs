    ...
    public class ShardingFileDocStorage : ObjectStorageBase, IFileStorage
    {
        ...
        public ObjectId Insert<T>(Stream context, T entity) where T : IMetaEntity, new()
        {
            return Excute(() =>
            {
                ...
                context.Position = 0;
                var fsInfo = GetDataBase(entity.Database).GridFS.Upload(context, entity.FileName);
        ...
        public void Delete<T>(IDictionary<string, object> condition) where T : IMetaEntity, new()
        {
            Excute(() =>
            {
                ...
                var list = GetCollection<T>().Find(query).AsQueryable().Select(p => new {p.Id, p.Database}).ToList();
                if (list.Count > 0)
                {
                    GetCollection<T>().Remove(new QueryDocument(DictionaryWarp(condition)));
                    list.ForEach(p=>GetDataBase(p.Database).GridFS.DeleteById(p.Id));
                }
        ...
        public Stream DownLoad(ObjectId id)
        {
            ...
            if (info == null)
            {
                throw new MongoException("文件不存在或文件已经归档。");
            }
        ...
        public bool DeleteAll<T>() where T : IMetaEntity, new()
        {
            return Excute(() =>
            {
                ...
                list.ForEach(p =>
                {
                    if (ParseWriteConcernResult(GetCollection<T>().Remove(Query.EQ("_id", p.Id))))
                    {
                        GetDataBase(p.Database).GridFS.DeleteById(p.Id);
                    }
                });
        ...
        public void DownLoad<T>(IDictionary<string, object> condition, string saveFile) where T : IMetaEntity, new()
        {
            ...
            var query = new QueryDocument(DictionaryWarp(condition));
            var entity = GetCollection<T>().Find(query).AsQueryable().Select(p => new{ p.Id ,p.Database}).FirstOrDefault();
            if (entity == null)
            {
                throw new FileNotFoundException();
            }

            MongoGridFSFileInfo info = GetDataBase(entity.Database).GridFS.FindOneById(entity.Id);
            if (info == null)
            {
            ...
            var stream = new MemoryStream();
            GetDataBase(entity.Database).GridFS.Download(stream, saveFile);
        }
        /// <summary>
        /// 下载文件
        /// </summary>
        ...
        public Stream DownLoad<T>(IDictionary<string, object> condition) where T : IMetaEntity, new()
        {
            ...
            var stream = new MemoryStream();
            GetDataBase(entity.Database).GridFS.Download(stream, info);
        ...
        public void DownLoad<T>(ObjectId id, string saveFile) where T : IMetaEntity, new()
        {
            ...
            var stream = new MemoryStream();
            GetDataBase(info.Database).GridFS.Download(stream, saveFile);
        }
        /// <summary>
        /// 查询字典数据转换程序
        /// 核心加入前缀和替换Id为_Id
