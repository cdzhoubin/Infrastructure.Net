...
namespace Zhoubin.Infrastructure.Common.MongoDb
{
    ...
    /// MongoDb文件存储
    /// </summary>
    public class FileDocStorage20 : ObjectStorageBase20, IFileStorage20
    {
        /// <summary>
        /// 构造函数
        /// </summary>
        /// <param name="configName">配置文件</param>
        public FileDocStorage20(string configName)
            : base(configName)
        {
        ...
        public ObjectId Insert<T>(Stream context, T entity) where T : IMetaEntity, new()
        {
            return Excute(() =>
            {
                ...
                var info = CheckGridFsFileExists(GridFS, fsInfo);
                entity.Id = fsInfo;
        ...
        public void Delete<T>(IDictionary<string, object> condition) where T : IMetaEntity, new()
        {
            Excute(() =>
            {
                ...
                if (list.Count > 0)
                {
                    GetCollection<T>().DeleteMany(new QueryDocument(DictionaryWarp(condition)));
        ...
        public Stream DownLoad<T>(ObjectId id) where T : IMetaEntity, new()
        {
            ...
            var fs = CheckGridFsFileExists(gridfs, info.Id);
            var stream = new MemoryStream();
