...
function PluploadProcessFile(uploadConfig) {
    ...
    this.container = $('#' + uploadConfig.ContainerId);
    var _uploadConfig = uploadConfig;
    var uploader = this.container.pluploadQueue({
        ...
        silverlight_xap_url: uploadConfig.PlUploadFolder + '/Moxie.xap',
        flash_swf_url: uploadConfig.PlUploadFolder + '/Moxie.swf',
    ...
    var _uploader = this.container.pluploadQueue();
    //加入启动下载开始方法，用于隐藏默认上传按钮后
    this.startUpload = function () {
        if (_uploader.files.length > 0) {
            _uploader.start();
            
        } else {
            if (uploadConfig.CompleteEvent) {
                uploadConfig.CompleteEvent();
            }
        }
    };
    //隐藏按钮方法
    this.hideButton = function(buttons) {
        if (buttons) {
            if (buttons.start != undefined && buttons.start != null && buttons.start == false) {
                this.container.find(".plupload_button.plupload_start").css("display", "none");
            }
            if (buttons.browse != undefined && buttons.browse != null && buttons.browse == false) {
                this.container.find(".plupload_button.plupload_add").css("display", "none");
            }
            if (buttons.stop != undefined && buttons.stop != null && buttons.stop == false) {
                this.container.find(".plupload_button.plupload_stop").css("display", "none");
            }
        }
    };
    //重设控件
    this.reset = function() {
        _uploader.destroy();
        var tmp = new PluploadProcessFile(_uploadConfig);
    };
}
