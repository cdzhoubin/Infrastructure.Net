...
namespace Zhoubin.Infrastructure.Common.MongoDb
{
    ...
    /// Mongodb对象存储
    /// </summary>
    public sealed class DocumentObjectStorage20 : ObjectStorageBase20, IObjectStorage
    {
        /// <summary>
        /// 构造方法
        /// </summary>
        /// <param name="configName">程序集配置名称</param>
        public DocumentObjectStorage20(string configName)
            : base(configName)
        {
        ...
        public T FindOneByCondition<T>(IDictionary<string, object> condition) where T : IDocumentEntity, new()
        {
            ...
            var result = Excute(() => GetCollection<T>().Find(new QueryDocument(condition))).FirstOrDefault();
            return result;
        }
        ...
        public List<T> FindByCondition<T>(IDictionary<string, object> condition, IDictionary<string, bool> sortList) where T : IDocumentEntity, new()
        {
            var result = Excute(() =>
            {
                ...
                sortList.ToList().ForEach(p => query = query.Sort(new SortByDocument(p.Key, new BsonInt32(p.Value ? 1 : -1))));
                return query.ToList();
            });
        ...
        {
            return Excute(() => condition == null || condition.Count == 0 ? GetCollection<T>().AsQueryable().Count() : GetCollection<T>().Count(new QueryDocument(condition)));
        }
